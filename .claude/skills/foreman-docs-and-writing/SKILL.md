...
|---|---|---|
| `CLAUDE.md` | Quick-load summary for Claude agents | **Most current summary.** Architecture list covers the major modules but not all 18 `src/*.rs` files. Two stale spots: the "Session Context" section names `feat/browser-style-tabs` as active though that work is merged to `main` (commits `d4da700`, `31a3db4`) and the current branch is `main`; its "Memory system" pointer names a directory that does not exist. |
