...
| 2 | **Stage files by name; never `git add -A` / `git add .`.** | Every executed plan staged by name. Incident: the agent-dispatch plan opened with a preflight task because the user had live uncommitted edits in the tree that `-A` "would sweep … into feature commits" — recover it with `git log --diff-filter=D --name-only -- docs/superpowers/plans/` (plans are deleted once their work ships). `.gitignore` covers build/tool dirs, `*.png`/`*.log`, and a few scratch artifacts (read it — it is short); scratch HTML (`_mockup.html`, `mocks/`) is tracked, and user WIP is never ignored. |
