...
| "Let ConPTY own the redraw" — tested, all four combinations failed | `docs/conpty-resize-reflow.md:51-63` | ConPTY's reported cursor is inconsistent with its own repaint (upstream microsoft/terminal #18725); no frontend redraw-ownership strategy can fix it. Full chronicle → **foreman-failure-archaeology**. |
...
| Ready latch = first DSR reply flush, NOT "DSR + output idle" | `docs/contracts/chat-handshake-remaining-work.md:29-33`; code at `src/terminal.rs:723-728` | A streaming agent never goes output-idle, so the idle variant never latches Ready. Note: `docs/contracts/chat-handshake-contract.md:74` still carries the older "post-DSR-settled" wording — superseded on this point. |
