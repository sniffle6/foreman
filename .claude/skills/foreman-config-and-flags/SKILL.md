...
|---|---|---|---|
| `PreToolUse` | `Bash` | `.claude/hooks/kill-foreman.ps1` | No-op when `FOREMAN=1` (session runs inside foreman — killing it would kill the host; incident 2026-07-09). Otherwise, if the command matches regex `cargo\s+(build\|run\|test)`: kill foreman **by exe path** (only instances under this repo's `target\`; by-name killed the user's installed foreman — incident 2026-07-15), sleep 500 ms. Always exits 0 (never blocks) |
