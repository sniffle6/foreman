...
`cargo build|run|test` command. It fires **only for the Bash tool** — a cargo
command issued through the PowerShell tool gets no kill, so run the
`Stop-Process` line yourself there. Build-loop details live in
