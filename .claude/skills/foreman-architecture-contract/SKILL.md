...
| Ordinary commits and PRs are not blocked by CI | `.github/workflows/test.yml` runs `cargo test` on pushes to `main` and on PRs, but only reports; nothing requires it to pass | Open. Local `cargo test` is still the gate for normal changes — **foreman-validation-and-qa** |
