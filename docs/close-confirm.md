...
**running child process**, foreman pops a modal first: it lists what will be
killed as `Name │ Pid` and makes you confirm. Close a terminal that's just
sitting at an idle prompt and nothing pops — it closes silently, like before.
...
1. **What counts as "running":** a shell's `root_pid` has at least one live
   ...
   (`OpenConsole.exe` / `conhost.exe` are filtered out). If the list is empty,
   the close happens immediately — no modal.
2. **The gate:** every *interactive* close routes through
...
3. **Rendering:** the modal is drawn in `show_modals`, which already runs at both
...
4. **The decision is split from the render:** `resolve_pending(outcome)` is a
   ...
   reports the outcome.
5. **The quit path** (title-bar X / Alt+F4) bypasses the window manager, so
...
- **The trigger policy is one function.** Want "warn only for agents" or "warn
  for any live shell" instead? Change `collect_descendants` in `src/proc.rs`;
  nothing else moves.
...
- `src/proc.rs` — `descendants(root_pid)` + the pure, tested `collect_descendants`
  (the trigger policy).
