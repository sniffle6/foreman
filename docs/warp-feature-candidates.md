...
warp.dev/blog/how-warp-works, warp.dev homepage) plus parallel subagent reviews,
each verified against the actual foreman codebase at the time.

This is a **backlog of open candidates**. Anything that has since shipped is
marked **SHIPPED** with a pointer to its feature doc — read those instead of
re-proposing the work. Code references name symbols, not line numbers; find them
with `rg 'fn <name>' src/`.
...
- **Interception seam exists:** foreman owns the raw PTY byte stream before
  ...
  `advance_scanned()` (`src/terminal.rs`), which already pre-scans bytes
...
- Injection: per-`Shell`-variant snippet (`enum Shell` in `src/terminal.rs`);
  install pattern precedent in `src/skills_install.rs`.
...
2. Shell spawn is bare (`CommandBuilder::new(shell.program())` in
   `Session::spawn`, no args/rc injection). `docs/shell-selection.md` pins a
...
**How it maps to foreman:**
- `src/control.rs`: `SnapshotRequest` gains opt-in flags (`last_command`, <!-- cite-guard: ok — proposed, not built -->

  `command: Option<i64>`), each `#[serde(default, skip_serializing_if = ...)]`;
  `parse_snapshot_args` gains CLI flags; `OpenReply` gains optional
  `exit_code`/`command_text`; output rides the existing generic
  `history: Option<Vec<String>>` payload. No new verb, no protocol v2. (`tail:
  Option<usize>` is the slice that already shipped — copy its shape.)
- `src/wm.rs`: `snapshot_dispatch` resolves the span; the `status` verb already
  reports `running` / `exited(code)` — but that's the *pane process*, not the
  last command.
- `src/inspect.rs`: pure seam; would gain `snapshot_range(term, start, end)`
  alongside `snapshot_text` / `snapshot_tail`.
- `src/terminal.rs`: `Session` stores the command-span table fed by the OSC 133
  sibling.

**Verified findings:**
- **Scrollback is reachable now:** `--tail N` shipped (`inspect::snapshot_tail`
  / `snapshot_cells_tail` + `tail_span`), so the default `snapshot_text`
  viewport walk is no longer the only read. A *range* read (`--region`,
  arbitrary start/end) still does not exist. (Related drift, fixed 2026-08-24:
  `control.rs` comments used to claim `--settle-ms` was "not yet honored". It
  is — the reply defers until the screen quiets, via `wm::advance_settles` /
  `settle_tick`. Don't re-report it.)
- **Scrollback is sliceable:** history depth is `Settings::scrollback_lines`
  (`src/config.rs`, user-editable and clamped); `src/graphics.rs`
  (`Graphics::visible`, the scroll-cull at the end of `apply`) already tracks
  content anchors across scroll via `history_size` deltas — precedent for
  buffer-stable "total line" coordinates.
...
- **Wire-compat v1 rules are explicit and paved:** optional fields +
  `skip_serializing_if` + a compat test — precedents in `src/control.rs`, e.g.
  `snapshot_reply_without_attrs_cursor_is_wire_compat`; list them with
  `rg 'wire_compat' src/control.rs`. Rules in foreman-change-control skill +
  `docs/adr/0001-...` (reply stays a presence-discriminated bag; reshaping is
  ask-first).
- Roadmap already points this way: `docs/epics/terminal-inspection-epic.md`
  reserves `--rows`, `--region`, `--wait-for`, `--since-seq`. Composes cleanly.

**Effort:** the standalone slice (scrollback `--tail N`) was the S half and
**shipped** — it needed no marks, exactly as predicted. What remains is the full
block model (span table, `--last-command`, `--failed`, per-command exit codes):
M, hard-depends on #1. Span bookkeeping across scroll/clear/reflow is the real
...
- The `--tail` slice (shipped) already pays for itself — long `cargo build`
  output no longer scrolls out of reach — and de-risked the rest.
...
- Scrollback truncation: a span whose head has aged out of
  `Settings::scrollback_lines` must degrade explicitly
...
**Verdict: ADOPT-LATER** — the `--tail` slice is done (`docs/terminal-inspection.md`);
the block model stays gated on #1 landing and an honest answer on alt-screen
coverage.
...
- **Backend is glow (OpenGL), not wgpu** — `default-features = false` on the
  `eframe` line in `Cargo.toml` is load-bearing, because eframe prefers wgpu
  whenever both are enabled. `egui-wgpu` `panic!`s unconditionally in
  `update_buffers` when a staging allocation fails, which is what Windows GPU
  device loss looks like on every sleep / display power transition; `egui_glow`
  only logs. Settled with a side-by-side A/B — `docs/gpu-device-loss.md`. Any
  renderer proposal inherits this constraint.
- **Repaint is already event-driven + adaptive:** the per-Session reader thread
  (spawned in `Session::spawn_with`, `src/terminal.rs`) calls
  `ctx.request_repaint()` per chunk; the timer is only a backstop — 4 ms while
  hot (<250 ms since activity), 100 ms idle (the `hot`/`cadence` tail of
  `App::update`, `src/main.rs`). Deliberate, measured
  (docs/followups-latency-and-control.md).
- **Text layout is memoized twice over** (`docs/galley-atlas.md`): a per-pane
  `MonoPaintCache` keyed on `MonoPaintKey{content_gen, off, cols, rows,
  font_bits, colors}` holds the built `MonoGlyph` list, and a Session-lived
  `MonoGalleyAtlas` dedupes galleys by `(char, GlyphStyle)` and survives a
  key miss. Blit positions are recomputed from live `CellMetrics`
  each frame, so the cache **survives scroll and resize** — the original
  whole-pane `LayoutJob`/`GalleyKey` scheme (2026-07-04), which rebuilt on every
  `display_offset` change, is gone.
- Only the active tab's content is drawn (`w.tabs[w.active]` throughout
  `src/wm.rs`); occluded floats are painted but cost ~an Arc clone when
  unchanged.
...
- Known small waste: a `"M".to_string()` + `layout_no_wrap` metrics probe per
  pane per frame (`src/terminal.rs`). The other item once listed here —
  "scroll rebuilds the whole-pane galley" — was fixed by the glyph atlas.

**Effort:** do nothing: S(zero). Targeted egui opts still open (bounded
per-frame VTE parsing, occlusion skip, hoist the metrics probe): S–M each. The
offset-independent glyph cache that used to head this list has **shipped**.
Custom renderer: **L–XL** — reimplements atlas/shaping/fallback/
...
- Scroll-storm in a 300-row 4K pane no longer re-*shapes* (the atlas holds), but
  it still rebuilds the `MonoGlyph` list and re-tessellates every frame — the
  one workload where a grid-native cell renderer is structurally better.
...
- `Session::read_input` (`src/terminal.rs`): keys → `input::process_input` →
  `encode_key` → PTY bytes. Foreman never holds or models a command line. Only
  chrome-level chords intercept (`src/input.rs`).
...
**How it maps to foreman:** `src/keymap.rs` — a flat `HashMap<Chord, Command>`
behind `resolve`; no context notion (only the cosmetic `Command::group()`).
Predicates would be evaluated in `WindowManager::pump_commands` / `dispatch`
(`src/wm.rs`).
...
- `pump_commands` already runs a hand-rolled predicate stack:
  ...
  settings.is_none() && no egui widget focused` — the last guard is unit-tested
  (`leader_stays_dormant_while_a_widget_holds_focus`).
- The leader state machine **swallows** its chords + stray Text events
  (`WindowManager::swallow_input`) so nothing leaks to `read_input`.
- Terminal input gated at render: `is_focus = focused == Some(id) && live &&
  picker/renaming/settings all None`, commented in place with the bug it
  prevents. Both modal-freeze behaviors are unit-tested.
- Similar ad-hoc guards: `overlay_blocks_close`, the `apply_acts` mouse bail,
  `app_modal` threading — all in `src/wm.rs`.
...
persistence surface: `(context, chord)` keying breaks
`resolve`/`chord_for`/`rebind`/`reset_one`/`save` (`src/keymap.rs`), the
settings editor's single-namespace conflict model, keybindings.json schema
(forward-compatible via `#[serde(default)]`, but a new-schema file fails on an
old build and the all-or-nothing `Keymap::load` silently discards the user's
whole custom map), and the module's test block —
`rg -c '#\[test\]' src/keymap.rs`.

**Pros:**
- Formalizes the hand-maintained boolean disjunctions into one source of truth.
...
- Injection path fully reusable: chat input strip (painted by `src/chat_view.rs`)
  → `ChatView::pending_post` (state in `src/chat.rs`) →
  `WindowManager::drain_chat_posts` (`src/wm.rs`) → … →
  **`Session::inject_input` (`src/terminal.rs`)** — the composer calls
  `inject_input` on its own Session directly; no chat room in the loop.
- `inject_input` is exactly the write primitive: queues in
  `ready_gate.pending_inject` until `ready` (avoids the boot-window swallow),
  wraps in `ready::paste_wrap()` (`ESC[200~…ESC[201~`, ESC-stripped so a quoted
  `ESC[201~` can't break out), defers the submitting `\r` by
  `ready::SUBMIT_DELAY` (Claude Code's burst detection folds a back-to-back
  `\r` into the paste — documented live failure 2026-06-10). The whole gate is
  now a pure seam in `src/ready.rs`.
- UI: mirror of the `docs/window-chrome.md` reserved-band pattern — reserve a
  ...
  the Session to whatever rect it gets. The chat input strip in
  `src/chat_view.rs` is a working template, including the egui Escape-defocus
  quirk.
...
- **Bracketed paste: already handled** — `paste_wrap` unconditional in v1,
  ...
  (`src/terminal.rs`) or keep composer opt-in on agent panes.
- **Focus precedent exists:** chat input already puts a TextEdit inside a wm
  ...
  intercept-keys-before-TextEdit (`src/dirpicker.rs`). The new bit: terminal keyboard reads are gated only by an `active` flag with **zero
  focus checks inside `Session::read_input`** (`src/terminal.rs`) — the wm must
  pass `active = focused && !composer_has_focus` or every composer keystroke also
...
- Real multi-line drafting with native selection/clipboard/undo — the chat
  input is **singleline** (`TextEdit::singleline` in `src/chat_view.rs`; Enter
  submits) and Claude Code's own TUI input under ConPTY doesn't give this.
...
- Existing signals: `Session::ready` + `Session::output_gen`
  ...
  `Settings::send_settle_ms`, `src/config.rs` — it ships at 120 ms and
  sanitize clamps it below `wm::MAX_SETTLE_MS`); agent *identity* via
  process-tree scan (`src/proc.rs`, throttled by `REFRESH_EVERY`, WSL-blind) +
  OSC-title fallback (`Session::osc_title`). `foreman status` reports
  running|exited(code) — no state column. **Gone since this was written:** the
  cursor-rest gate (`caret.rs` `CaretGate` / `CURSOR_SETTLE`) was deleted;
  `src/caret.rs` is now just `draw`. Don't plan around it.
- **Crew board exists as the overview seed:** `ChatRoom::crew()` + `CrewRow`
  (`src/chat.rs`), rendered as "CREW · BY LAST HEARD" by `src/chat_view.rs`
  inside `Content::Chat`, with click-to-focus via deferred actions in
  `src/wm.rs`. Per-project, chat-members-only today.
- Content-variant pattern confirmed: `enum Content` (`src/wm.rs`) already
  carries more than the original Terminal/Project/Chat trio — read the enum,
  don't trust a list here. A desktop-level `Content::Fleet` follows the same
  shape; wrinkle: it must read across project boundaries.
- Badge sites: the tab strip already paints per-tab icons (`Content::icon_kind`
  in `src/wm.rs` → `src/icons.rs`) — a state dot slots in beside; headers are
  quiet chrome, so a badge must be a small glyph/dot, not loud fill.
...
- **Zero async** — no Tokio, no smol, no executor. The dep list has grown since
  this was written (network I/O now exists for the update check, still blocking
  on a plain thread); read it with
  `sed -n '/^\[dependencies\]/,/^\[dev-dependencies\]/p' Cargo.toml`.
- Thread model: **1 detached reader thread per Session** (blocking read into a
  64 KiB stack buffer → mpsc → `request_repaint`, the thread spawned in
  `Session::spawn_with`; PTY writes are synchronous on the GUI thread — no
  writer thread). Control plane: 1 listener + short-lived per-connection
  threads capped at `MAX_INFLIGHT` (`src/control.rs`). Roughly one thread per
  terminal plus a small constant — single-digit MB committed, parked in
  blocking ReadFile, exactly conhost's own idiom. Even 500 sessions would be
  fine.
- Wakeups are event-driven; all sleep loops found are `#[cfg(test)]` only.
- **Font fallback was the actual gap — now SHIPPED** (`docs/font-fallback.md`).
  It needed no font-kit, exactly as predicted: `terminal_font::load_font_definitions`
  best-effort `std::fs::read`s `C:\Windows\Fonts\msyh.ttc` (YaHei) and
  `seguiemj.ttf` (Segoe UI Emoji) and **appends** them after the defaults;
  missing file = skip, don't crash. Color emoji went further than the original
  sketch — egui 0.34's atlas is outline-only, so `src/emoji_raster.rs` stamps
  DirectWrite/D2D color bitmaps on top. The *data model* was already
  wide-char-correct (WIDE_CHAR_SPACER, CJK selection acid-tested per
  docs/terminal-selection.md); this was only ever a glyph-coverage bug.
- One acknowledged wart: a wedged control-handler thread is reclaimed only when
  its client dies (interprocess' sync stream exposes no read timeout — see the
  comment above `MAX_INFLIGHT` in `src/control.rs`) — bounded by the cap.
...
- **Font fallback itself: SHIPPED** — the one validated gap, closed.
  `docs/font-fallback.md`.

**Open questions:** the color-emoji question is answered — `src/emoji_raster.rs`
stamps color bitmaps over egui's outline-only atlas. What's left: cross-platform
font discovery, and only if foreman ever leaves Windows.
