...
impl Painter {
    ...
    fn configure_surface(
        ...
        // FORK: upstream `.expect(...)`s here, which turns a lost adapter into
        // a SECOND crash inside the recovery path. Both callers can reach this
        // on a dead adapter — `recreate_surface` -> `install_surface` ->
        // `resize_and_generate_depth_texture_view_and_msaa_view`, and the
        // `needs_reconfigure` branch that a display-topology change across a
        // resume can set from a Resized event — so hardening the function
        // covers both at once.
        let Some(default_config) =
            ...
            log::warn!(
                "egui-wgpu (foreman fork): surface unsupported by adapter \
                 (device or adapter lost); skipping configure"
            );
            return;
        };
