// LOCAL FORK of egui-wgpu 0.34.3. Only deviation from upstream: device-lost
// handling — see docs/gpu-device-loss.md. Re-derive the diff with
// `diff -ru <registry copy> vendor/egui-wgpu` before any eframe bump.
...
impl Renderer {
    /// FORK: (byte-range-within-buffer, bytes) for every mesh's indices, in
    /// paint order. Callback primitives contribute nothing and do NOT advance
    /// the offset — identical to the upstream loop this replaces.
    ///
    /// This exists so the staging path and the device-lost fallback path in
    /// `update_buffers` push IDENTICAL slices. That invariant is what keeps
    /// `Renderer::render` safe, and it is exactly what a future rebase would
    /// break, so it is tested (`mod fork_tests`).
    fn index_slices(
        paint_jobs: &[epaint::ClippedPrimitive],
    ) -> impl Iterator<Item = (Range<usize>, &[u8])> {
        let mut offset = 0usize;
        paint_jobs.iter().filter_map(move |cp| match &cp.primitive {
            Primitive::Mesh(mesh) => {
                // Upstream computes `mesh.indices.len() * size_of::<u32>()`,
                // which is exactly `bytes.len()`.
                let bytes: &[u8] = bytemuck::cast_slice(&mesh.indices);
                let slice = offset..offset + bytes.len();
                offset += bytes.len();
                Some((slice, bytes))
            }
    ...
    /// FORK: the vertex twin of [`Renderer::index_slices`]; same contract.
    fn vertex_slices(
        paint_jobs: &[epaint::ClippedPrimitive],
    ) -> impl Iterator<Item = (Range<usize>, &[u8])> {
        let mut offset = 0usize;
    ...
    pub fn update_buffers(
        ...
        if index_count > 0 {
            ...
            match index_buffer_staging {
                Some(mut staging) => {
                    for (slice, bytes) in Self::index_slices(paint_jobs) {
                        staging.slice(slice.clone()).copy_from_slice(bytes);
                        self.index_buffer.slices.push(slice);
                    }
                }
                None => {
                    // FORK: upstream panics here. `write_buffer_with` returns
                    // None ONLY on device loss: wgpu-29.0.3
                    // src/backend/wgpu_core.rs:1976-2008 routes every error
                    // through handle_error_nolabel, and handle_error_inner
                    // (:281-299) returns early ONLY for ErrorType::DeviceLost —
                    // Internal / OutOfMemory / Validation all reach
                    // default_error_handler, which panics before we could get
                    // here. If that ever changes upstream this warn will fire
                    // on a non-lost device; treat it as a bug, not noise.
                    //
                    // The slices MUST still be pushed: `slices` was cleared
                    // above and `Renderer::render` `.expect()`s an entry per
                    // mesh. An early `return` would just relocate the crash.
                    crate::device_loss::mark_lost();
                    log::warn!(
                        "egui-wgpu (foreman fork): index staging buffer unavailable — wgpu \
                         device lost. Falling back to queue.write_buffer. index_count={index_count}"
                    );
                    let mut data = Vec::with_capacity(required_index_buffer_size as usize);
                    for (slice, bytes) in Self::index_slices(paint_jobs) {
                        data.extend_from_slice(bytes);
                        self.index_buffer.slices.push(slice);
                    }
                    // Cannot panic on a lost device: wgpu-29.0.3
                    // backend/wgpu_core.rs write_buffer -> handle_error_nolabel
                    // -> DeviceLost early return.
                    queue.write_buffer(&self.index_buffer.buffer, 0, &data);
                }
            }
        }
        if vertex_count > 0 {
            ...
            match vertex_buffer_staging {
                Some(mut staging) => {
                    for (slice, bytes) in Self::vertex_slices(paint_jobs) {
                        staging.slice(slice.clone()).copy_from_slice(bytes);
                        self.vertex_buffer.slices.push(slice);
                    }
                }
                None => {
                    // FORK: same story as the index block above — upstream
                    // panics; a `None` here means the wgpu device is lost.
                    crate::device_loss::mark_lost();
                    log::warn!(
                        "egui-wgpu (foreman fork): vertex staging buffer unavailable — wgpu \
                         device lost. Falling back to queue.write_buffer. \
                         vertex_count={vertex_count}"
                    );
                    let mut data = Vec::with_capacity(required_vertex_buffer_size as usize);
                    for (slice, bytes) in Self::vertex_slices(paint_jobs) {
                        data.extend_from_slice(bytes);
                        self.vertex_buffer.slices.push(slice);
                    }
                    queue.write_buffer(&self.vertex_buffer.buffer, 0, &data);
                }
            }
        }
...
// FORK: GPU-free tests for the shared slice helpers introduced by the
// device-loss fallback. Their whole purpose is that the staging path and the
// `queue.write_buffer` fallback push byte-identical `slices` entries; a rebase
// that "simplifies" one of the two loops back to an inline offset counter is
// exactly what these pin.
#[cfg(test)]
mod fork_tests {
    use super::*;
    use epaint::{ClippedPrimitive, Mesh, PaintCallback, TextureId, emath::Rect};
    use std::sync::Arc;

    fn mesh(indices: usize) -> ClippedPrimitive {
        let mut m = Mesh::with_texture(TextureId::default());
        m.indices = (0..indices as u32).collect();
        // Two vertices per index keeps the vertex ranges distinct from the
        // index ranges, so a copy-paste slip between the two helpers shows up.
        m.vertices = vec![Vertex::zeroed(); indices * 2];
        ClippedPrimitive {
            clip_rect: Rect::ZERO,
    ...
    #[test]
    fn index_slices_are_contiguous_and_skip_callbacks() {
        let jobs = vec![mesh(2), callback(), mesh(3)];
        let got: Vec<_> = Renderer::index_slices(&jobs).collect();
        // The callback contributes nothing AND does not advance the offset.
        assert_eq!(got.len(), 2);
        assert_eq!(got[0].0, 0..8); // 2 * size_of::<u32>()
        assert_eq!(got[1].0, 8..20); // 3 * size_of::<u32>(), contiguous
        assert_eq!(got[0].1.len(), 8);
        assert_eq!(got[1].1.len(), 12);
    }

    #[test]
    fn vertex_slices_are_contiguous_and_skip_callbacks() {
        let v = std::mem::size_of::<Vertex>();
        let jobs = vec![mesh(2), callback(), mesh(3)];
        let got: Vec<_> = Renderer::vertex_slices(&jobs).collect();
        assert_eq!(got.len(), 2);
        assert_eq!(got[0].0, 0..(4 * v));
        assert_eq!(got[1].0, (4 * v)..(10 * v));
    }

    #[test]
    fn both_paths_push_the_same_index_slices() {
        let jobs = vec![mesh(1), callback(), mesh(7), mesh(2)];
        let a: Vec<Range<usize>> = Renderer::index_slices(&jobs).map(|(r, _)| r).collect();
        let b: Vec<Range<usize>> = Renderer::index_slices(&jobs).map(|(r, _)| r).collect();
        assert_eq!(a, b);
        // And the concatenated bytes exactly fill the last slice's end — the
        // property the fallback's `Vec` relies on.
        let total: usize = Renderer::index_slices(&jobs).map(|(_, b)| b.len()).sum();
        assert_eq!(total, a.last().unwrap().end);
    }

    #[test]
    fn both_paths_push_the_same_vertex_slices() {
        let jobs = vec![mesh(1), callback(), mesh(7), mesh(2)];
        let a: Vec<Range<usize>> = Renderer::vertex_slices(&jobs).map(|(r, _)| r).collect();
        let b: Vec<Range<usize>> = Renderer::vertex_slices(&jobs).map(|(r, _)| r).collect();
        assert_eq!(a, b);
        let total: usize = Renderer::vertex_slices(&jobs).map(|(_, b)| b.len()).sum();
        assert_eq!(total, a.last().unwrap().end);
    }
}
