...
/// FORK ADDITION: device-loss signal, set at the exact site where the failure
/// is first observable. This is the RELIABLE detector.
/// `Device::set_device_lost_callback` is not: wgpu-core documents the closure
/// "might never be called" (device/global.rs:1995-2007) and `device_destroy`
/// (device/global.rs:2019-2031) sets `valid = false` without invoking it.
pub mod device_loss {
    ...
    /// Sticky: a wgpu Device that has been lost is permanently invalid
    /// (wgpu-core device/resource.rs:669-679), so this never clears.
    pub fn device_lost() -> bool {
        LOST.load(Ordering::Acquire)
    }
}
pub use device_loss::device_lost;
